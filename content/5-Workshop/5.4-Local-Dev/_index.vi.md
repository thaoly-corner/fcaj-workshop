...
- File `.env` đã tạo từ `.env.example` (xem [Section 5.2 Prerequisites](5.2-Prerequisites/))
...
# Xóa virtual environment (tuỳ chọn)
deactivate
rm -rf venv/
```
...
```bash
# Kiểm tra không còn containers
docker compose ps
# Kết quả mong đợi: không có services nào

# Kiểm tra volumes đã xóa
docker volume ls | grep newsrag
# Kết quả mong đợi: không có volumes nào
```
