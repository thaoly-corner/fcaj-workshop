...
- **AWS Permissions**: VPC, EC2, RDS, ECS, ECR, IAM, CloudWatch, EventBridge
- **Bedrock model access**: `amazon.titan-embed-text-v2:0` enabled in AWS Console
- **API Keys**: Groq API Key, Google Gemini API Key
- **`terraform.tfvars`** created with values (see Section 5.2)
...
```bash
# Confirm no ECS clusters remain
aws ecs list-clusters

# Confirm no RDS clusters remain
aws rds describe-db-clusters

# Confirm no EventBridge rules remain
aws events list-rules --name-prefix newsrag
```

> **WARNING:** This destroys the Aurora cluster and **all data permanently**. Backup first if needed.
