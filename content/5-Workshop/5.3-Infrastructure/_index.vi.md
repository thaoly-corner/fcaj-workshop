...
- **Quyền AWS**: VPC, EC2, RDS, ECS, ECR, IAM, CloudWatch, EventBridge
- **Bedrock model access**: Đã bật `amazon.titan-embed-text-v2:0` trong AWS Console
- **API Keys**: Groq API Key, Google Gemini API Key
- **File `terraform.tfvars`** đã tạo và điền giá trị (xem section 5.2)
...
Xác minh đã xóa hết tài nguyên:

```bash
# Kiểm tra không còn ECS cluster
aws ecs list-clusters

# Kiểm tra không còn RDS cluster
aws rds describe-db-clusters

# Kiểm tra không còn EventBridge rules
aws events list-rules --name-prefix newsrag
```
