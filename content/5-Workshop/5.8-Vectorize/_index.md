...
- ETL completed — `fact_chunks` has data in PostgreSQL (Section 5.7 ETL)
- Qdrant running (`docker compose ps qdrant`, port 6333)
...
2. Run vectorization:
   ...
   ```
   Process: read chunks from `fact_chunks` → generate embeddings via SentenceTransformer → upsert to Qdrant collection `news_chunks`.

3. Check log output — lines like `Upserted N vectors to Qdrant` confirm vectors were saved.

## Test & Validation

```bash
# Check collection stats on Qdrant
...
Expected result: Response contains `vectors_count` > 0, collection `news_chunks` exists with correct embedding dimension (384 for BGE-small).

## Clean-up

None. Vectorization only writes data to Qdrant; no temporary resources created.
