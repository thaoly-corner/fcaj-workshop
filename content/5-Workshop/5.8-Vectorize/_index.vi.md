...
- ETL đã hoàn thành — `fact_chunks` có dữ liệu trong PostgreSQL (Section 5.7 ETL)
- Qdrant đang chạy (`docker compose ps qdrant`, port 6333)
...
2. Chạy vector hóa:
   ...
   ```
   Quy trình: đọc chunks từ `fact_chunks` → tạo embeddings bằng SentenceTransformer → upsert vào Qdrant collection `news_chunks`.

3. Kiểm tra log output — dòng `Upserted N vectors to Qdrant` xác nhận vectors đã được lưu.

## Test & Validation

```bash
# Kiểm tra collection stats trên Qdrant
curl -s http://localhost:6333/collections/news_chunks | python -m json.tool

# Hoặc dùng Qdrant Dashboard
# Mở http://localhost:6333/dashboard
```

Kết quả mong đợi: Response chứa `vectors_count` > 0, collection `news_chunks` tồn tại với đúng embedding dimension (384 cho BGE-small).

## Clean-up

Không cần clean-up. Vectorization chỉ ghi data vào Qdrant, không tạo resource tạm.
