...
- Raw data present in PostgreSQL table `article_metadata` (Section 5.6 Ingestion completed)
...
2. Run the ETL:
   ...
   ```
   The ETL transforms data from `article_metadata` into Star Schema tables (`dim_source`, `dim_time`, `dim_content`, `dim_author`, `fact_articles`, `fact_chunks`).

3. Check log output — lines like `ETL processed: <title>...` confirm each article was transformed.
...
-- Check row counts in Star Schema tables
...
Expected result: All dimension/fact tables contain data, join query returns correct results.

## Clean-up

None. ETL only writes data to the database; no temporary resources created.
