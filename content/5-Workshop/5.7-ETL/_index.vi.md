...
- Dữ liệu thô đã có trong PostgreSQL table `article_metadata` (hoàn thành Section 5.6 Ingestion)
...
2. Chạy ETL:
   ...
   ```
   ETL sẽ transform dữ liệu từ `article_metadata` vào Star Schema (`dim_source`, `dim_time`, `dim_content`, `dim_author`, `fact_articles`, `fact_chunks`).

3. Kiểm tra log output — dòng `ETL processed: <title>...` xác nhận mỗi bài đã được transform.
...
-- Kiểm tra số lượng dữ liệu trong các bảng Star Schema
...
-- Sample join query kiểm tra mối quan hệ
SELECT a.article_id, s.source_name, t.date_day, c.title
...
Kết quả mong đợi: Mỗi bảng dimension/fact có dữ liệu, join query trả về kết quả đúng.

## Clean-up

Không cần clean-up. ETL chỉ ghi dữ liệu vào database, không tạo resource tạm.
