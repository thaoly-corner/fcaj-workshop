...
- Terraform infrastructure đã deploy (xem [Section 5.3](5.3-Infrastructure/))
- Docker image đã build và push lên ECR (xem [Section 5.9](5.9-AWS-Deploy/))
- AWS CLI đã cấu hình
- ECS Cluster `newsrag-cluster` đã tồn tại
...
CLUSTER=$(terraform output -raw ecs_cluster_name)
TASK_ARNS=$(aws ecs list-tasks --cluster $CLUSTER --family newsrag-crawler --desired-status RUNNING --query "taskArns[]" --output text)
for ARN in $TASK_ARNS; do
  aws ecs stop-task --cluster $CLUSTER --task $ARN
done

# Lưu ý: Dọn dẹp hoàn toàn qua terraform destroy (xem Section 5.17)
```
