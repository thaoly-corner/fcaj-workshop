...
aws ecr describe-images --repository-name newsrag-api --region ap-southeast-2
```
**Kết quả mong đợi:** Ảnh với tag `latest` tồn tại.

### 2. Kiểm tra Lambda Functions
```bash
aws lambda list-functions --query "Functions[?starts_with(FunctionName, 'newsrag')].FunctionName" --region ap-southeast-2
```
**Kết quả mong đợi:** `newsrag-consumer`, `newsrag-etl`, `newsrag-rag-api`

### 3. Kiểm tra Secrets Manager
```bash
aws secretsmanager get-secret-value --secret-id newsrag/db-credentials --region ap-southeast-2
```

### 4. Test deploy script
```bash
cd /home/tuan/AWS-Project
bash scripts/deploy.sh
```
**Kết quả mong đợi:** Terraform apply thành công, Docker build thành công, ECR push thành công.
...
# Xóa Docker images cục bộ
docker rmi news-crawler:latest 2>/dev/null
docker rmi $(docker images -q -f dangling=true) 2>/dev/null

# Lưu ý: Tài nguyên AWS thực tế được xóa qua terraform destroy (xem Section 5.17)
```
...
**Tiếp theo:** [Fargate Crawler](5.10-Fargate/)
