...
- Docker services running (`docker compose ps` — PostgreSQL, Kafka, Zookeeper)
- Python virtual environment activated
- Dependencies installed (`pip install -r requirements.txt`)
- Kafka topic `news_raw` exists (see Section 5.4)
...
## Step-by-step (Running the Crawler)
...
```bash
# Check output.json for extracted articles
...
Expected: JSON objects with article fields appearing in the topic.
...
# Press Ctrl+C in the terminal running scrapy crawl
