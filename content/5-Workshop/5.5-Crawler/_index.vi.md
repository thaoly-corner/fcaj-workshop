...
- Docker services đang chạy (`docker compose ps` — PostgreSQL, Kafka, Zookeeper)
- Python virtual environment đã activate
- Dependencies đã cài (`pip install -r requirements.txt`)
- Kafka topic `news_raw` đã tồn tại (xem [Section 5.4 Local Dev](5.4-Local-Dev/))
...
### Xác minh kết quả trích xuất

```bash
# Kiểm tra output.json chứa bài báo đã trích xuất
...
Kết quả mong đợi: các JSON object chứa fields bài viết xuất hiện trong topic.
...
# Nhấn Ctrl+C trong terminal đang chạy scrapy crawl

# Xóa Docker image
docker rmi news-crawler:local

# Xóa file output
rm -f crawler/output.json
```

Xác minh đã dọn dẹp:

```bash
# Kiểm tra không còn container crawler
docker ps | grep news-crawler
# Kết quả mong đợi: không có containers nào
```
