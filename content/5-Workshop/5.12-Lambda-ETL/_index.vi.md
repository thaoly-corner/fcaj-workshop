...
- Aurora PostgreSQL với pgvector extension đã enable
- Dữ liệu thô đã có trong `article_metadata` (từ Lambda Consumer)
- Bedrock model `amazon.titan-embed-text-v2:0` đã bật quyền truy cập
- Lambda Consumer đã hoạt động (xem [Section 5.11](5.11-Lambda-Consumer/))
...
```
**Kết quả mong đợi:** `statusCode: 200` với kết quả ETL.

### 2. Kiểm tra log
```bash
aws logs tail /aws/lambda/newsrag-etl --follow
```
**Kết quả mong đợi:** `ETL complete: X articles processed, Y chunks created, Z embeddings generated`

### 3. Xác minh Star Schema trong CSDL
```bash
ENDPOINT=$(terraform output -raw rds_endpoint)
psql "postgresql://postgres:password@${ENDPOINT}:5432/newsrag" -c "
...
UNION ALL SELECT 'fact_articles', count(*) FROM fact_articles
UNION ALL SELECT 'fact_chunks', count(*) FROM fact_chunks;
"
```

### 4. Xác minh Embeddings
```bash
psql "postgresql://postgres:password@${ENDPOINT}:5432/newsrag" -c "
SELECT count(*) as total_chunks,
       count(embedding) as embedded_chunks,
       pg_size_pretty(pg_total_relation_size('fact_chunks')) as table_size
FROM fact_chunks;
"
```
...
```bash
# Không có tài nguyên tạm thời cần dọn cho bước này
# Lambda function được xóa qua terraform destroy (xem Section 5.17)

# Tùy chọn: reset trạng thái ETL
ENDPOINT=$(terraform output -raw rds_endpoint)
psql "postgresql://postgres:password@${ENDPOINT}:5432/newsrag" -c "
DELETE FROM fact_chunks;
DELETE FROM fact_articles;
DELETE FROM dim_content;
