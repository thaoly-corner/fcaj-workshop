...
- Aurora PostgreSQL with pgvector extension enabled
- Raw data in `article_metadata` table (from Lambda Consumer)
- Bedrock model `amazon.titan-embed-text-v2:0` access enabled
- Lambda Consumer operational (see Section 5.11)
...
aws logs tail /aws/lambda/newsrag-etl --follow
```
**Expected:** `ETL complete: X articles processed, Y chunks created, Z embeddings generated`

### 3. Verify Star Schema in Database
```bash
ENDPOINT=$(terraform output -raw rds_endpoint)
psql "postgresql://postgres:password@${ENDPOINT}:5432/newsrag" -c "
...
```bash
# No temporary resources to clean for this step
# Lambda function cleaned via terraform destroy (see Section 5.17)

# Optionally reset ETL state
ENDPOINT=$(terraform output -raw rds_endpoint)
psql "postgresql://postgres:password@${ENDPOINT}:5432/newsrag" -c "
