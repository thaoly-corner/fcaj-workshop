...
- Aurora pgvector đã có dữ liệu embeddings (xem [Section 5.12](5.12-Lambda-ETL/))
- Bedrock model `amazon.titan-embed-text-v2:0` đã bật
...
```
**Kết quả mong đợi:** JSON với `summary`, mảng `results` (5 phần tử có scores), `duration_ms`.

### 3. Test vanilla (không RAG)
```bash
curl -X POST "$API_URL" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
# Không có tài nguyên tạm thời cho bước này
# API Gateway + Lambda được xóa qua terraform destroy (xem Section 5.17)

# Tùy chọn: xóa log streams test
aws logs delete-log-stream \
  --log-group-name /aws/lambda/newsrag-rag-api \
