...
- Aurora PostgreSQL deployed and accessible (see Section 5.3)
- SQS queue `newsrag-news-raw` exists
- Database schema initialized (`database/warehouse.sql`)
- Crawler running and sending messages to SQS
...
Send the same message again — should see `Duplicate skipped` in logs.

## Clean-up

```bash
# Delete test messages from SQS
QUEUE_URL=$(aws sqs get-queue-url --queue-name newsrag-news-raw --query "QueueUrl" --output text)
aws sqs purge-queue --queue-url $QUEUE_URL

# Note: Lambda function cleaned via terraform destroy (see Section 5.17)
```
