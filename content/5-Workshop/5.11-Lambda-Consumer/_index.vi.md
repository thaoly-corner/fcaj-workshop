...
- Aurora PostgreSQL đã deploy và accessible (xem [Section 5.3](5.3-Infrastructure/))
- SQS queue `newsrag-news-raw` đã tồn tại
- Database schema đã khởi tạo (`database/warehouse.sql`)
- Crawler đang chạy và gửi messages vào SQS
...
### 2. Kiểm tra log Lambda
```bash
aws logs tail /aws/lambda/newsrag-consumer --follow
```
**Kết quả mong đợi:** `Inserted article: https://vnexpress.net/test-article-123...`

### 3. Xác minh trong CSDL
```bash
ENDPOINT=$(terraform output -raw rds_endpoint)
psql "postgresql://postgres:password@${ENDPOINT}:5432/newsrag" -c "
SELECT source_name, title, embedded, created_at
...
### 4. Test trùng lặp
Gửi lại cùng một tin nhắn — sẽ thấy `Duplicate skipped` trong log.

## Clean-up

```bash
# Xóa tin nhắn test từ SQS
QUEUE_URL=$(aws sqs get-queue-url --queue-name newsrag-news-raw --query "QueueUrl" --output text)
aws sqs purge-queue --queue-url $QUEUE_URL

# Lưu ý: Lambda function được xóa qua terraform destroy (xem Section 5.17)
```
